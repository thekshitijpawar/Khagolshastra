...
function cleanAudioUrl(rawUrl: string): string {
  if (!rawUrl) return ''
...
// 3-Day Rotation Math: Starts at Ep. 1 on base epoch (August 31, 2026), advancing every 3 days (72 hours) to Ep. 2, Ep. 3, ... currently Ep. 8 on Sep 21, 2026.
function getActiveRotatingPodcast(): PodcastEpisode {
  const EPOCH_MS = new Date('2026-08-31T00:00:00Z').getTime()
  const nowMs = Date.now()
  const periodMs = 3 * 24 * 60 * 60 * 1000 // 72 hours (3 days)
